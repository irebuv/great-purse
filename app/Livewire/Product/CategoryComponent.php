...
{
    use WithPagination, CartTrait, WishTrait;
    public string $slug = '';
    ...
    #[Url]
    public int $limit = 20;
    public array $limitList = [5, 10, 20, 50];
    ...
    {
        $this->limit = in_array($this->limit, $this->limitList) ? $this->limit : $this->limitList[2];
        $this->resetPage();
    }
    ...
    public function render()
    {
        ...
        //$ids = CategoryHelpers::getIds($category->id) . $category->id;
        $ids = $category->id;
        ...
        $min_range = Product::when($all_products, function (Builder $query) use ($ids) {
            $query->whereIn('category_id', explode(',', $ids));
        })->min('price');

         $max_range = Product::when($all_products, function (Builder $query) use ($ids) {
            $query->whereIn('category_id', explode(',', $ids));
        })->max('price');
        ...
        if (is_null($this->min_price) || is_null($this->max_price)) {
            $min_max_price = DB::table('products')
                ...
                ->when($all_products, function (QueryBuilder $query) use ($ids) {
                    $query->whereIn('category_id', explode(',', $ids));
                })
                ->get();
        ...
        $products = Product::query()
            ->when($all_products, function (Builder $query) use ($ids) {
                $query->whereIn('category_id', explode(',', $ids));
            })
            ->when($this->selected_filters, function (Builder $query) use ($cnt_filter_groups) {
                $query->leftJoin(
                    DB::raw('filter_products FORCE INDEX FOR JOIN (filter_id)'),
        ...
        if ($page > $products->lastPage()){
            $this->redirectRoute('category', $category->slug);
            //$this->resetPage();
        }
        $this->categoryTitle = $category->title;
        $title = "Category {$category->title}" . ($page ? " :: Page - {$page}" : '');

        $breadcrumbs = CategoryHelpers::getBreadCrumbs($category->id);
